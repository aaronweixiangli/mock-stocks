...
async function getStock(req, res) {
  try {
    ...
    const stock = await fetch(
      `https://api.twelvedata.com/time_series?apikey=${Twelve_Data_API_Key}&interval=${interval}&format=JSON&timezone=America/Los_Angeles&symbol=${symbol}&start_date=${startDate}&dp=2`
    ).then((res) => res.json());
    res.json(stock);
  } catch (err) {
    ...
    if (buyOrSell === "buy" && sharesOrDollars === "shares") {
      ...
      try {
        ...
        if (user.balance - dollars >= 0) {
          ...
          if (stockOwn) {
            // New total cost = (current average cost per share * current number of shares) + (number of new shares * purchase price per share)
            const newTotalCost =
              stockOwn.avgCost * stockOwn.qty +
              buyInQuantity * currentMarketPrice;
            // New total number of shares = current number of shares + number of new shares
            const newQuantity = Number(
              (stockOwn.qty + buyInQuantity).toFixed(5)
            );
            // New average cost per share = New total cost / New total number of shares
            const newAvgCost = newTotalCost / newQuantity;
            stockOwn.qty = newQuantity;
            ...
            await user.save();
            return res.json({
              success: `Market order (buy in dollars) to buy $${dollars} of ${symbol} has been successfully executed. You've received ${buyInQuantity} shares at an average price of $${currentMarketPrice} per share`,
            });
          } else {
      ...
      try {
        ...
        if (stockOwn && stockOwn.qty - shares >= 0) {
          ...
          // Succesfully place the order, update user's balance
          user.balance = Number(
            (user.balance + shares * currentMarketPrice).toFixed(2)
          );
          await user.save();
          ...
          return res.json({
            success: `Market order (sell in shares) to sell ${shares} shares of ${symbol} has been successfully executed at an average price of $${currentMarketPrice} per share.`,
          });
      ...
      try {
        ...
        const currentMarketPrice = Number(stockData.values[0].open);
        const sellInQuantity = Number(
          (dollars / currentMarketPrice).toFixed(5)
        );
        // if user owns the stock and the stock shares are greater than or equal to the sell in shares, execute the order
        if (stockOwn && stockOwn.qty - sellInQuantity >= 0) {
          ...
          const newTotalCost =
            stockOwn.avgCost * stockOwn.qty -
            sellInQuantity * currentMarketPrice;
          // New total number of shares = current number of shares - number of shares sold
          const newQuantity = Number(
            (stockOwn.qty - sellInQuantity).toFixed(5)
          );
          // New average cost per share = New total cost / New total number of shares
          const newAvgCost = newTotalCost / newQuantity;
          stockOwn.qty = newQuantity;
          ...
          return res.json({
            success: `Market order (sell in dollars) to sell $${dollars} of ${symbol} has been successfully executed. You've sold ${sellInQuantity} shares at an average price of $${currentMarketPrice} per share`,
          });
          // If user does not own the stock or does not have enought shares to sell, create notification instance
        } else {
          await Notification.create({
    ...
    if (buyOrSell === "buy" && sharesOrDollars === "shares") {
      try {
        ...
        if (user.balance - cost >= 0) {
          ...
          return res.json({
            success: `Your market order (buy in shares) to buy ${shares} shares of ${symbol} has been placed successfully but cannot be executed now since the market is closed. If this order isn't filled by the end of market hours (4:00pm ET) on the next trading day, it will expire.`,
          });
        } else {
          // If user's balance is less than the cost
          // create a notification instance for this user
      ...
      try {
        ...
        if (user.balance - dollars >= 0) {
          ...
          return res.json({
            success: `Your market order (buy in dollars) to buy $${dollars} of ${symbol} has been placed successfully but cannot be executed now since the market is closed. If this order isn't filled by the end of market hours (4:00pm ET) on the next trading day, it will expire.`,
          });
        } else {
      ...
      try {
        ...
        if (stockOwn && stockOwn.qty - shares >= 0) {
          ...
          return res.json({
            success: `Your market order (sell in shares) to sell ${shares} shares of ${symbol} has been placed successfully but cannot be executed now since the market is closed. If this order isn't filled by the end of market hours (4:00pm ET) on the next trading day, it will expire.`,
          });
        } else {
          // create a notification instance for this user
          await Notification.create({
      ...
      try {
        ...
        if (stockOwn && stockOwn.qty - sellInQuantity >= 0) {
          ...
          return res.json({
            success: `Your market order (sell in dollars) to sell $${dollars} of ${symbol} has been placed successfully but cannot be executed now since the market is closed. If this order isn't filled by the end of market hours (4:00pm ET) on the next trading day, it will expire.`,
          });
        } else {
          // create a notification instance for this user
          await Notification.create({
...
async function placeLimitOrder(req, res) {
  ...
  if (buyOrSell === "buy") {
    ...
    if (user.balance - cost >= 0) {
      ...
      await user.save();
      await Notification.create({
        text: `We've received your limit order to buy ${shares} shares of ${symbol} at a maximum of $${limitPrice} per share. ${
          expires === "good for day"
            ? `If this order isn't filled by the end of merket hours (4:00pm ET) ${
                isMarketOpen ? `today` : `on the next trading day`
              }, it will expire.`
      ...
      return res.json({
        success: `Your limit order to buy ${shares} shares of ${symbol} at a maximum of $${limitPrice} per share has been placed successfully. 
                ${
                  expires === "good for day"
                    ? `If this order isn't filled by the end of merket hours (4:00pm ET) ${
                        isMarketOpen ? `today` : `on the next trading day`
    ...
    if (stockOwn && stockOwn.qty - shares >= 0) {
      ...
      await stockOwn.save();
      await Notification.create({
        text: `We've received your limit order to sell ${shares} shares of ${symbol} at a minimum of $${limitPrice} per share. ${
          expires === "good for day"
            ? `If this order isn't filled by the end of merket hours (4:00pm ET) ${
                isMarketOpen ? `today` : `on the next trading day`
              }, it will expire.`
      ...
      return res.json({
        success: `Your limit order to sell ${shares} shares of ${symbol} at a minimum of $${limitPrice} per share has been placed successfully. ${
          expires === "good for day"
            ? `If this order isn't filled by the end of merket hours (4:00pm ET) ${
                isMarketOpen ? `today` : `on the next trading day`
              }, it will expire.`
