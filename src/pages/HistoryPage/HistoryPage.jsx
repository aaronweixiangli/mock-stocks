...
export default function HistoryPage({ user }) {
  ...
  function toggleOrderDetails(evt) {
    ...
    const childContainer = parentContainer.querySelector(
  ...
  if (pendingOrder && Array.isArray(pendingOrder)) {
    pending = pendingOrder.map((order) => (
      <section
        className="order-all-container"
        onClick={toggleOrderDetails}
        key={order._id}
      >
        <div className="order-content">
          <div className="order-left">
            <div className="order-type bold">
              {order.symbol}{" "}
              {order.orderType === "market order" ? "Market" : "Limit"}{" "}
              {order.buyOrSell === "buy" ? "Buy" : "Sell"}
            </div>
        ...
        <div className="order-detail-container">
          <div className="order-detail-grid">
            <div className="order-detail-card">
              ...
              <span className="order-detail-content green bold">
                {order.symbol}
              </span>
            </div>
            <div className="order-detail-card">
              <span className="order-detail-title">Type</span>
              <span className="order-detail-content">
                {order.orderType === "market order" ? "Market" : "Limit"}{" "}
                {order.buyOrSell === "buy" ? "Buy" : "Sell"}
              </span>
            </div>
      ...
      <div className="order-content" key={h._id}>
        <div className="order-left">
          <div className="order-type bold">
            {h.category === "buy"
              ? `${h.symbol} ${
                  h.orderType === "market order" ? "Market" : "Limit"
                } Buy`
              : h.category === "sell"
              ? `${h.symbol} ${
                  h.orderType === "market order" ? "Market" : "Limit"
            ...
            {h.category === "deposit" ? (
              ...
              <>
                <div className="order-price bold">
                  ${Number(h.marketPrice * h.qty).toFixed(2)}
                </div>
                <div className="order-qty">
                  {h.qty} shares at ${h.marketPrice}{" "}
                </div>
              </>
            )}
          </div>
