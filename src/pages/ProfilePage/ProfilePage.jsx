...
export default function ProfilePage( { user, balance, setBalance, balanceOnHold, setBalanceOnHold  } ) {
            ...
            <section className='profile-container'>
                <div className='profile-user-container'>
                    <div className='profile-logo'>
                        <img src="https://i.imgur.com/mSyRgSe.png" alt="mockstocklogo2" className='mockstock-logo2'/>
                    </div>
                    <div className='profile-user-content'>
                        <p>{user.name}</p>
                        <p>{user.email} • <span className='profile-join-year'>Joined {(new Date(user.createdAt)).toLocaleString("en-US", {timeZone: "America/Los_Angeles", year: "numeric", })}</span> </p> 
                    </div>
                </div>
                <div className='total-in-mockstocks'>
                    <p> {String(brokerageHolding).startsWith('Unable') ? `$${(balance + balanceOnHold ).toFixed(2)}. (Brokerage holdings are currently not being considered. Please refresh the page at a later time.)` : `$${(balance + balanceOnHold + brokerageHolding).toFixed(2)}`} </p>
                ...
                <div className='profile-investing-container'>
                    <div className='profile-investing-content'>
                        ...
                        <p className='bold'>{String(brokerageHolding).startsWith('Unable') ? `$${(balance + balanceOnHold ).toFixed(2)}` : `$${(balance + balanceOnHold + brokerageHolding).toFixed(2)}`} </p>
                    </div>
                    <div className='profile-investing-content'>
                        <span>Brokerage holdings</span>
                        <span className='profile-investing-amount'>${brokerageHolding}</span>
                    </div>
                    <div className='profile-investing-content'>
                        <span>Brokerage cash</span>
                        <span className='profile-investing-amount'>${(balance + balanceOnHold).toFixed(2)}</span>
                    </div>
                    <div className='profile-investing-content'>
