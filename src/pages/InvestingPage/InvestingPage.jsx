...
export default function InvestingPage({
  ...
  let stocks = null;
  if (
    typeof stocksHolding !== "string" &&
    Array.isArray(stocksHolding.stockOwn)
  ) {
    stocks = stocksHolding.stockOwn.map((s, index) => (
      <tr key={index} onClick={() => navigate(`/stocks/${s.symbol}`)}>
        ...
        <td>
          {" "}
          <div className="stock-total-return">
            <div
              className={
                (stocksHolding.currentMarketPrices[s.symbol] - s.avgCost) *
                  s.qty >=
                0
                  ? "return-indicator-up"
            ...
            $
            {Math.abs(
              (stocksHolding.currentMarketPrices[s.symbol] - s.avgCost) * s.qty
            ).toFixed(2)}
          </div>
        </td>
        <td>
          $
          {Number(s.qty * stocksHolding.currentMarketPrices[s.symbol]).toFixed(
            2
          )}
        </td>
      </tr>
  ...
  if (
      ...
      datasets: [
        {
          label: "value",
          data: [
            Number(stocksHolding.brokerageHolding.toFixed(2)),
            Number((Number(balance) + Number(balanceOnHold)).toFixed(2)),
          ],
          backgroundColor: ["coral", "#8BC34A"],
          borderWidth: 1,
        },
      ],
    };
    stocksData = {
      ...
      datasets: [
        {
          label: "equity",
          data: stocksHolding.stockOwn.map((s) =>
            Number(s.qty * stocksHolding.currentMarketPrices[s.symbol]).toFixed(
              2
            )
          ),
    ...
    <main className="InvestingPage">
      ...
      <section className="investing-container">
        <div className="investing-portfolio">
          ...
          <div className="total-portfolio-value-amount bold">
            {stocksHolding
              ? `$${(
                  Number(stocksHolding.brokerageHolding) +
                  Number(balance) +
                  Number(balanceOnHold)
                ).toFixed(2)}`
              : "loading..."}
          </div>
          <ul className="total-portfolio-categories">
            <li className="total-portfolio-category">
              ...
              <div className="category-value">
                <span className="gray-color">
                  {(
                    (100 * Number(stocksHolding.brokerageHolding)) /
                    (Number(stocksHolding.brokerageHolding) +
                      Number(balance) +
                      Number(balanceOnHold))
                  ).toFixed(2)}
            ...
            <li className="total-portfolio-category">
              ...
              <div className="category-value">
                <span className="gray-color">
                  {(
                    (100 * Number(balance + balanceOnHold)) /
                    (Number(stocksHolding.brokerageHolding) +
                      Number(balance) +
                      Number(balanceOnHold))
                  ).toFixed(2)}
                ...
                <span>
                  ${(Number(balance) + Number(balanceOnHold)).toFixed(2)}
                </span>
              </div>
            </li>
          </ul>
        ...
        <table className="investing-stocks-table">
          ...
          <tbody className="investing-stocks-body-container">{stocks}</tbody>
        </table>
      </section>
      {portfolioData ? (
        <div className="portfolio-doughnut-chart">
          <Doughnut data={portfolioData} />
        </div>
      ) : null}
      {stocksData ? (
        <div className="stocks-doughnut-chart">
          <Doughnut data={stocksData} />
        </div>
      ) : null}
    </main>
  );
