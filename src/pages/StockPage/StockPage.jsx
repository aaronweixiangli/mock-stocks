    ...
    return (
      <main className="StockPage">
        ...
        <h2>
          Symbol not found. <br />
          <br />
          Invalid input: {symbol}. <br />
          <br />
          Please double-check and try again. <br />
          <br />
          Remember to enter the stock symbol instead of the company name. For
          example, instead of searching for APPLE, use the stock symbol AAPL.
        </h2>
        <h2>
          Sometimes it can be the network issue. If the input is valid, try
          refreshing the page later.
        </h2>
      </main>
    );
  }
    ...
    <main className="StockPage">
      <h1 className="stockpage-symbol-name">
        ...
        <button
          ...
          onClick={() => toggleStockWatch(symbol)}
        >
      ...
      {stockData && Array.isArray(stockData.values) ? (
        <>
          ...
          <div className="chart-btns-container">
            <button
              className={`chart-btn ${chartBtn === "1D" ? "active" : ""}`}
              onClick={handleOneDay}
            >
              1D
            </button>
            <button
              className={`chart-btn ${chartBtn === "1W" ? "active" : ""}`}
              onClick={handleOneWeek}
            >
              1W
            </button>
            <button
              className={`chart-btn ${chartBtn === "1M" ? "active" : ""}`}
              onClick={handleOneMonth}
            >
              1M
            </button>
            <button
              className={`chart-btn ${chartBtn === "3M" ? "active" : ""}`}
              onClick={handleThreeMonths}
            >
              3M
            </button>
            <button
              className={`chart-btn ${chartBtn === "1Y" ? "active" : ""}`}
              onClick={handleOneYear}
            >
              1Y
            </button>
            <button
              className={`chart-btn ${chartBtn === "5Y" ? "active" : ""}`}
          ...
          <StockOrder
            symbol={symbol}
            marketPrice={stockData.values[0].open}
            user={user}
            sharesOwn={sharesOwn}
            setSharesOwn={setSharesOwn}
            sharesOnHold={sharesOnHold}
            setSharesOnHold={setSharesOnHold}
            balance={balance}
