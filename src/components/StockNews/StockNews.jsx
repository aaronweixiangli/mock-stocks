...
export default function StockNews({ symbol }) {
      ...
      <a href={n.url} className="news-container" key={index}>
        <div className="news-content-left">
          <span className="news-time">
            {n.source} &nbsp; {getTimeDiff(n.time_published)}
          </span>
          <p>{n.title}</p>
          <div className="news-ticker">
            {n.ticker_sentiment.map((t, idx) => (
              <span
                className="ticker_sentiment"
                key={idx}
                onClick={(evt) => handleClick(evt, t.ticker)}
              >
                {t.ticker}
              </span>
            ))}
  ...
  return (
    <>
      <h1>News</h1>
      {news ? (
        <section className="stock-news-container">{news_card}</section>
      ) : (
        <p>
          Loading.... if it loads more than 30 seconds, try refreshing the page.
        </p>
      )}
    </>
  );
