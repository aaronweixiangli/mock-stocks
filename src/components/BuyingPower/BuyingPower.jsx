...
export default function BuyingPower({
  ...
  function toggleSection() {
    document
      .getElementById("buying-power-section")
      .classList.toggle("show-section");
    // adjust the location of the scroll btns in 'Learn About Investing' section
    document
      .getElementsByClassName("card-scroll-right-btn")[0]
      .classList.toggle("btn-move-down");
    document
      .getElementsByClassName("card-scroll-left-btn")[0]
      .classList.toggle("btn-move-down");
  }
  ...
  async function handleDeposit(evt) {
    ...
    const updatedBalance = await usersAPI.deposit(
      ...
      <div className="buying-power-container" onClick={toggleSection}>
        ...
        <div id="buying-power-section">
          <div className="left-section">
            <div className="buying-power-content">
              <p>Brokerage Cash</p>
              <p>${(balance + balanceOnHold).toFixed(2)}</p>
            </div>
            <div className="buying-power-content">
              <p>Pending Orders</p>
              <p>-${balanceOnHold.toFixed(2)}</p>
            </div>
            <div className="buying-power-content">
              <p>Buying Power</p>
              <p className="bold-font">${balance.toFixed(2)}</p>
            </div>
            <button className="show-modal-btn" onClick={toggleModal}>
              Deposit Funds
            </button>
          </div>
          <div className="right-section">
            <p>
              Buying power represents the total value of assets you can
              purchase.
            </p>
          </div>
        </div>
