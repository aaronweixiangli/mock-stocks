    ...
    if (order.orderType === "market order") {
      ...
      if ((orderDay === 5 && dayDiff > 3) || (orderDay === 6 && dayDiff > 2)) {
        ...
        await Notification.create({
          text: `Your market order to ${buyOrSell === "buy" ? "buy" : "sell"} ${
            shares ? `${shares} shares` : `$${dollars}`
          } of ${symbol} placed on ${new Date(order.createdAt).toLocaleString(
            "en-US",
            {
      ...
      if (expires === "good for day") {
        if (isMarketOpen) {
          ...
          if (
            (orderDay === 5 && dayDiff > 3) ||
            (orderDay === 6 && dayDiff > 2)
          ) {
            orderExpired = true;
      ...
      if (orderExpired) {
        ...
        // Create notification for user that the order has expired
        await Notification.create({
          text: `Your limit order to ${buyOrSell === "buy" ? "buy" : "sell"} ${
            shares ? `${shares} shares` : `$${dollars}`
          } of ${symbol} at ${
            buyOrSell === "buy" ? "maximum" : "minimum"
          } $${limitPrice} per share placed on ${new Date(
            order.createdAt
